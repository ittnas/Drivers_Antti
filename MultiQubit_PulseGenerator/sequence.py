...
class SequenceToWaveforms:
                ...
                if isinstance(gate_obj, (gates.IdentityGate, gates.VirtualZGate)):
                    ...
                    if self.compensate_crosstalk:
                        crosstalk = self._crosstalk.compensation_matrix[:,qubit]
                                            ...
                                            gates.RabiGate, 
                                            gates.SingleQubitXYRotation_12,
                                            gates.SpinlockingGate)):
    ...
    def set_parameters(self, config={}):
        ...
        for n, pulse in enumerate(self.pulses_1qb_xy):
            ...
            pulse.iq_skew = config.get('XY gates IQ skew #%d' % m) * np.pi / 180
            pulse.iq_ratio = config.get('XY gates I/Q ratio #%d' % m)

            self.pulses_1qb_xy[n] = pulse

        # spin-locking gates
        for n, pulse in enumerate(self.pulses_1qb_SL):
            m = n + 1  # pulses are indexed from 1 in Labber
            pulse = (getattr(pulses, config.get('SL - Pulse type'))
                     (complex=True))
            # global parameters
            pulse.truncation_range = config.get('SL - Truncation range')
            pulse.start_at_zero = True
            pulse.use_drag = True

            # pulse shape
            pulse.width = config.get('SL - Width')
            pulse.plateau = config.get('SL - Plateau')
            pulse.amplitude = config.get('SL - Amplitude')
            
            # pulse-specific parameters
            pulse.frequency = config.get('SL - Frequency')
            pulse.drag_coefficient = config.get('SL - DRAG scaling')
            pulse.drag_detuning = 0

            pulse.iq_skew = config.get('SL - IQ skew') * np.pi / 180
            pulse.iq_ratio = config.get('SL - I/Q ratio')

            self.pulses_1qb_SL[n] = pulse

        # single-qubit pulses XY for 1-2 transition
        for n, pulse in enumerate(self.pulses_1qb_xy_12):
            m = n + 1  # pulses are indexed from 1 in Labber
            pulse = (getattr(pulses, config.get('Pulse type 12'))
                     (complex=True))
            # global parameters
            pulse.truncation_range = config.get('Truncation range 12')
            pulse.start_at_zero = config.get('Start at zero 12')
            pulse.use_drag = True#config.get('Use DRAG 12')
            # pulse shape
            if config.get('Uniform pulse shape 12'):
                pulse.width = config.get('Width 12')
                pulse.plateau = config.get('Plateau 12')
            else:
                pulse.width = config.get('Width 12 #%d' % m)
                pulse.plateau = config.get('Plateau 12 #%d' % m)

            if config.get('Uniform amplitude 12'):
                pulse.amplitude = config.get('Amplitude 12')
            else:
                pulse.amplitude = config.get('Amplitude 12 #%d' % m)

            # pulse-specific parameters
            pulse.frequency = config.get('Frequency 12 #%d' % m)
            pulse.drag_coefficient = config.get('DRAG scaling 12 #%d' % m)
            pulse.drag_detuning = config.get('DRAG frequency detuning 12 #%d' % m)

            pulse.iq_skew = config.get('XY gates IQ skew 12 #%d' % m) * np.pi / 180
            pulse.iq_ratio = config.get('XY gates I/Q ratio 12 #%d' % m)
